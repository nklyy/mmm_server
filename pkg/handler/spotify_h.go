...
func (h *Handler) spotifyAuthRedirect(ctx *fiber.Ctx) error {
	...
	return ctx.Redirect("https://accounts.spotify.com/authorize?response_type=code&client_id=6b990a58d275455da234d248fda89722&scope=" + scope + "&redirect_uri=" + r + "&show_dialog=true&state=" + m + "," + questId)
}
...
func (h *Handler) spotifyCallback(ctx *fiber.Ctx) error {
	...
	return ctx.Redirect("http://localhost:3000/cf?type=s&code=" + token + "&m=" + splitState[0] + "&qi=" + splitState[1])
}
